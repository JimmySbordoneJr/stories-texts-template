...
<div class="hidden">
	...
	<?php 
	// If there are other versions of the story, this sets up the carousel, which lets users slide through the versions. 
	...
	<!-- This is the area covered by an image, and it contains the title, description, and the videos -->
	<div class="jumbotron jumbotron-fluid">
		<div class="container">
				...
				<div class="col-md-6 col-sm-12">
					<div class="black-box black-box--notTitle">
						...
						<div class="container-fluid">
							<div class="row">				
								<?php 
									// This prints the names of the speakers and provide a link to other stories they have told.
										if(strpos($narrator, ",") !== FALSE){
											...
											for($n = 0; $n < count($narrator_array); $n++){
												...
												echo '<div class="col-lg-9 col-xs-8"><a class="text-light" target="_blank" href="http://yourwebsite.com/texts/index.php?lang=translation&searchTerm=Speaker:' . str_replace(" ", "_", $narrator_array[$n]) . '">' . $narrator_array[$n] . '</a></div>';
											}
										} else{
											echo '<div class="col-lg-3 col-xs-4">Speaker:</div>';
											// 10. Replace with your website name.
											echo '<div class="col-lg-9 col-xs-8"><a class="text-light" target="_blank" href="http://yourwebsite.com/texts/index.php?lang=translation&searchTerm=Speaker:' . str_replace(" ", "_", $narrator) . '">' . $narrator . '</a></div>';
										}
									?>
							</div>
						</div>
			...
			<!-- Between lines 202 and 247, we build the video panel. -->
				<div class="col-md-6 col-sm-12">
	...
	<div class="container-fluid">
		<div class="row">
			<div class="col">
				...
				<!-- This button opens and closes all of the glosses at once. It works using a Javascript file below -->
	...
	<!-- Here comes a huge PHP loop! We are building the story text section. -->
	<?php 
		$i = 1;
		...
		if($lines->length > 0){
			foreach($lines as $currentLine){
				...
				if(($currentLine->getElementsByTagName("morpheme")->length > 0) && (trim($currentLine->getElementsByTagName("translation")->item(0)->nodeValue) !== "")){
					...
					// foreach loop to handle the morphemes
					$bagOfMorphemes = $currentLine->getElementsByTagName("morpheme");
					if($bagOfMorphemes->length > 0){ // if this line has morphemes...
					...
					// if the line has no morphemes to show, but does have a translation filled out...
				...
				}else{// if a line has no morphemes and no translation to show...
					echo '<button class="btn btn-sm btn-invisible" disabled>Gloss</button>';
					echo $hintText . $currentLine->getElementsByTagName("speaker")->item(0)->nodeValue . '</div></div></div> <div class="col"><p class="mainSourceLang">' . $currentLine->getElementsByTagName("source")->item(0)->nodeValue . '</p></div>';
				}
	...
	if($otherVersions->length > 1){
		...
		foreach($otherVersions as $nextStory){
			...
			if($nextStory->getAttribute('id') != $id){
				...
				// Here, we build the jumbotron-- title, description, videos
				echo '<div class="jumbotron jumbotron-fluid">
							...
							<div class="row">
								<div class="col-md-6 col-sm-12">
									<div class="black-box black-box--notTitle">
										...
										if(strpos($nextNarrator, ",") !== FALSE){
											...
											for($n = 0; $n < count($narrator_array); $n++){
												...
												echo '<div class="col-lg-9 col-xs-8"><a class="text-light" target="_blank" href="http://yourwebsite.com/texts/secretIndex.php?translation&searchTerm=Speaker:' . str_replace(" ", "_", $narrator_array[$n]) . '">' . $narrator_array[$n] . '</a></div>';
											}
										} else{
											echo '<div class="col-lg-3 col-xs-4">Speaker:</div>';
											// 14. Replace with your website name
											echo '<div class="col-lg-9 col-xs-8"><a class="text-light" target="_blank" href="http://yourwebsite.com/texts/secretIndex.php?translation&searchTerm=Speaker:' . str_replace(" ", "_", $nextNarrator) . '">' . $nextNarrator . '</a></div>';
										}
								
									echo '</div></div></div></div>'; // This is end of the title and description half of the top of the page
										// This is the start of the video half of the top of the page
									...
									<nav>
										...
										if($nextYoutubelinkboth !== ""){
											echo '<li class="nav-item"><a class="nav-link active" id="SourceLangTranslationLang-tab' . $storyCount2 . '" data-toggle="pill" href="#SourceLangTranslationLang' . $storyCount2 . '" role="tab" aria-controls="SourceLangTranslationLang' . $storyCount2 . '" aria-selected="true">Both Subtitles</a></li>';			
										}
					...
					echo '<div class="container-fluid">
						...
						<br/>';	
					// Creating the story text section
						$i = 1;
						$lines = $nextMorphogloss->getElementsByTagName("line");
						foreach($lines as $currentLine){
						$hintText = "<button type=\"button\" class=\"btn btn-sm btn-invisible\" disabled><i class=\"fas fa-lightbulb\"></i></button>";
						// If the hint is not empty, we create a button, add it to hintText, and create a modal to display the text 
						if(trim($currentLine->getElementsByTagName("note")->item(0)->nodeValue) != ""){
						...
						if(($currentLine->getElementsByTagName("morpheme")->item(0)->nodeValue->length > 0) && (trim($currentLine->getElementsByTagName("translation")->item(0)->nodeValue) !== "")){
							...
							echo $hintText . $currentLine->getElementsByTagName("speaker")->item(0)->nodeValue . 
							'</div></div></div> 
				...
				echo '</div></div>';				// close a new container and carousel item div
				$storyCount2++;
			}
		}
		echo '</div>
	...
	foreach($modalInfo as $buildingModal){
		echo '<div class="modal fade" id="' . $buildingModal[0] . '" tabindex="-1" role="dialog" aria-labelledby="' . $buildingModal[0] . 'Label" aria-hidden="true">
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								<div class="modal-header">
									<h5 class="modal-title" id="' . $buildingModal[0] . 'Label">Note</h5>
